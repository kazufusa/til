...
// rely on `summary` alone. responseMimeType + responseSchema force the model
// into this shape, so we never have to strip ```markdown fences or guess
// what came back.
...
// describeImages: filename → formatted markdown caption.
//
// For `kind=='table'` images, the caption is `summary\n\n<gfm table>` so the
// post-processor can drop the table into the document as a sibling block
// below the `> **[画像]**` blockquote.
//
// Same image referenced multiple times → only one LLM call (dedup by
// filename). Unsupported mimes (emf/wmf/...) skip the call.
export async function describeImages(
  ...
  // Dedup by filename
  const unique = new Map<string, Image>();
  for (const img of images) {
    if (!unique.has(img.filename)) unique.set(img.filename, img);
  }
  ...
  for (const img of skipped) {
    out.set(img.filename, `(未対応の画像形式: ${img.mimeType})`);
  }
  ...
  for (let i = 0; i < callable.length; i += CONCURRENCY) {
    ...
    chunk.forEach((img, idx) => out.set(img.filename, results[idx]!));
  }
  return out;
}
