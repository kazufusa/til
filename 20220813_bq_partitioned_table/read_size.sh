...
bq query --nouse_legacy_sql --dry_run "
SELECT * FROM \`${PROJECT_ID}.${DATASET_US}.users\` WHERE created_at < \"2020-12-31\"
"


# + bq query --nouse_legacy_sql --dry_run
# SELECT * FROM `gcp-terraform-github.dataset.users`
#
# Query successfully validated. Assuming the tables are not modified, running this query will process 126 bytes of data.
# + bq query --nouse_legacy_sql --dry_run
# SELECT * FROM `gcp-terraform-github.dataset.users` WHERE created_at < "2020-12-31"
#
# Query successfully validated. Assuming the tables are not modified, running this query will process 45 bytes of data.
