...
**確定スコア(clean N=50×3 / paired / 低エラー)**:
| | 完全正答(=2) | partial+ | mean(0-2) |
|---|---|---|---|
| 改善前(narrow) | 53.4% | 85% | 1.385 |
| **改善後(wide)** | **60.7%** | **91%** | **1.520** |
| paired delta | — | — | **+0.127 ± 0.099 (有意✅)** |
- 部品: 日本語 keyword Hit@10 **0%→96.6%**(決定的)。英語(cross-lingual)完全正答 ~76-85%(検索 vector 90%)。
