...
## EXP-013 ON 忠実版(EVAL_ON: 分解→各サブで検索+“テキスト抽出”→テキスト統合) 〔実行中〕
- P1-3(chunk_id 抽出=けち)も DECOMP(抽出なし=ノイズ)も ON 本体でない。ON の核心=**各サブを焦点テキストに脱ノイズしてから統合**を初めて実装(`runOnAnswer`)。
- 出典は ON 同様持たない(品質が wide を超えるか先に測る)。judge は最終テキスト採点。paired vs wide で判定。
