...
- **検索**: file / 見出し / キーワード(全文) / ベクトル(pgvector) / 前後展開 をツールとして持つ
- **出典**: 回答はブロックに分かれ、各ブロックに根拠チャンクを対応付け。チップをクリックすると右ペインで **元 markdown の該当箇所(char offset)をハイライト**表示
- **スキル**: `skills/*.md` を投入すると実行可能スキルになる。実行されると、通常のソース参照に **加えて** スキルファイルへの参照を記録(`skill_invocations`)し、UI にも表示
...
markdowns/*.md ─ chunk.ts(構造認識: 見出し→ブロック, char offset 保持)
              └ embed.ts ─ embedding-model.ts(:embedContent アダプタ)─ Vertex
                 → chunks(content, char_start/end, heading_path, embedding halfvec(3072), tsv)

質問 ─ /api/chat
   1) runSearchAgent: tools[search_files, search_headings, keyword_search,
                            vector_search, expand_chunk, list_skills, run_skill,
                            select_sources] をループ実行 → 採用チャンク + 実行スキル確定
   2) streamAnswer(streamObject + zod): { blocks:[{ text, citations:[chunkId] }] } を stream
   → references(chunks + skills)を先に、続けて answer を NDJSON で配信

UI(page.tsx): 左=チャット(ブロック+引用チップ), 右=プレビュー
   引用チップ → /api/sources/:id の生 markdown を char_start..end でハイライト
   skill 参照 → /api/skills/:id を表示
```

### DB スキーマ(`db/init/01_schema.sql`)

- `sources`: markdown 全文(プレビュー基準) + メタ
- `chunks`: ブロック単位。`char_start/end`、`heading_path[]`、`embedding halfvec(3072)`、`tsv`(全文検索)
  - index: HNSW(`halfvec_cosine_ops`)、GIN(`tsv`)、GIN(`heading_path`)、trigram(`heading_text`)
...
| `chunks` | チャンク本体 | `source_id`, `ordinal`, `block_type`, `heading_path text[]`, `heading_text`, `char_start/end`, `content`, `embedding halfvec(3072)`, `tsv tsvector`(生成列) |
...
インデックス: `chunks` に HNSW(`halfvec_cosine_ops`)/ GIN(`tsv`)/ GIN(`heading_path`)/ trigram(`heading_text`)、`(source_id, ordinal)` btree。
...
| `keyword_search` | `tsv @@ websearch_to_tsquery('english', q)`、`ts_rank_cd` 降順 | 固有名詞・コマンド名に強い全文検索 |
...
- ハイブリッド融合は「スコア式の固定 RRF」ではなく **エージェントの逐次判断**で行う(クエリごとに使うツール・回数を変える)。
